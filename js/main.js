...
// ==========================================
// DATA ANALYTICS PORTFOLIO - CLEANED SCRIPT
// ==========================================
...
// --- HERO ANIMATION (Slide into view) ---
// This is the code that makes your hero appear. 
// It is now safe to run because there are no errors above it.
window.onload = function () {
  const items = document.querySelectorAll('.slide-into-view');
...
// --- SCROLL TRIGGER FOR CONTACT SECTION (Keep this) ---
const contactSection = document.getElementById('contact');
if (contactSection) {
  window.addEventListener('scroll', () => {
    const contactPosition = contactSection.getBoundingClientRect().top;
...
// --- TESTIMONIAL CAROUSEL (Keep this) ---
const testimonials = document.querySelectorAll('.testimonial-card');
...
// --- FAQ FUNCTIONALITY (Keep this) ---
document.querySelectorAll('.faq-title').forEach(title => {
  title.addEventListener('click', function () {
    const faqItem = this.parentElement;
    const faqContent = faqItem.querySelector('.faq-content');
    const faqIcon = faqItem.querySelector('.faq-toggle-icon');
    if (faqContent) faqContent.classList.toggle('hidden');
    if (faqIcon) faqIcon.textContent = faqContent.classList.contains('hidden') ? '+' : '-';
  });
});